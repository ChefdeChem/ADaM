...
import { hasSpellcastingFocus, setLightSourceMode } from "../src/engine/equipment-actions";
...
export default function Home() {
  ...
  function choosePendingOpportunityAttack(attackId: string | null) {
    ...
    setResolutionReceipt(buildResolutionReceipt({ kind: "opportunity-attack", before: encounter, after: result.encounter, actorId: playerCombatant.id, summary: result.summary, concealEnemyHitPoints: experienceMode === "advanced" }));
    setEnemyTurnPhase(result.encounter.pendingResponse ? "awaiting-player" : "resolving");
  }

  function rollPendingOpportunityAttack() {
    const pending = encounter.pendingResponse?.type === "opportunity-attack" ? encounter.pendingResponse : null;
    const attack = pending?.attackId ? playerCombatant.attacks.find((candidate) => candidate.id === pending.attackId) : undefined;
    const target = pending ? encounter.combatants.find((combatant) => combatant.id === pending.targetCombatantId) : undefined;
    ...
    if (result.playerRoll) {
      ...
      setRollExplanation(explainD20Roll({
        kind: "attack",
        title: `${attack?.name ?? "Opportunity Attack"} against ${target?.name ?? "target"}`,
        roll: result.playerRoll,
        target: experienceMode === "advanced" || !target ? undefined : { label: `${target.name} AC`, value: effectiveArmorClass(encounter, target.id) },
        hiddenTargetLabel: experienceMode === "advanced" ? "Target AC" : undefined,
        outcome: result.encounter.pendingResponse?.type === "opportunity-attack" && result.encounter.pendingResponse.phase === "damage-roll" ? "Hit" : "Miss",
        nextStep: result.encounter.pendingResponse?.type === "opportunity-attack" ? "Roll damage to complete the reaction." : "The interrupted movement continues.",
      }));
    }
    setFeedback(result.summary);
    setResolutionReceipt(buildResolutionReceipt({ kind: "opportunity-attack", before: encounter, after: result.encounter, actorId: playerCombatant.id, summary: result.summary, concealEnemyHitPoints: experienceMode === "advanced" }));
  ...
  function rollPendingOpportunityDamage() {
    ...
    if (result.damageRoll) {
      setLastRoll(result.damageRoll);
      setRollExplanation(explainDamageRoll({
        title: `${attack?.name ?? "Opportunity Attack"} damage`,
        roll: result.damageRoll,
        targetName: target?.name ?? "The target",
        nextStep: result.encounter.pendingResponse ? "Resolve the highlighted follow-up response." : "The enemy turn continues if the target survived.",
      }));
    }
    setFeedback(result.summary);
    setResolutionReceipt(buildResolutionReceipt({ kind: "opportunity-attack", before: encounter, after: result.encounter, actorId: playerCombatant.id, summary: result.summary, concealEnemyHitPoints: experienceMode === "advanced" }));
  ...
  function changeLightSource(itemId: string, mode: "off" | "bright" | "hooded") {
    const result = setLightSourceMode(encounter, playerCombatant.id, itemId, mode);
    if (!result.legal) {
      setFeedback(experienceMode === "advanced" ? "Action disallowed." : result.reason);
      return;
    }
    setEncounter(result.encounter);
    setFeedback(result.reason);
  ...
  function voluntarilyReleaseGrapple(effectId: string) {
    ...
    setEncounter(result.encounter);
    setFeedback(result.summary);
    setResolutionReceipt(buildResolutionReceipt({ kind: "release-grapple", before: encounter, after: result.encounter, actorId: playerCombatant.id, summary: result.summary }));
  }

  function stopHidingBySpeaking() {
    const next = endHiding(encounter, playerCombatant.id, "player speaks loudly");
    const summary = "You speak above a whisper and stop hiding.";
    setEncounter(next);
    setFeedback(summary);
    setResolutionReceipt(buildResolutionReceipt({ kind: "reveal-self", before: encounter, after: next, actorId: playerCombatant.id, summary }));
  }
  ...
  function runAction(action: CombatAction) {
    ...
    if (feature) {
      ...
      setToolFlow(null);
      setFeedback(result.summary);
      if (feature.id === "divine-sense") setResolutionReceipt(buildResolutionReceipt({ kind: "divine-sense", before: encounter, after: result.encounter, actorId: playerCombatant.id, summary: result.summary }));
      return;
    }
    ...
    if (action.id === "dodge") setResolutionReceipt(buildResolutionReceipt({ kind: "dodge", before: encounter, after: next, actorId: playerCombatant.id, summary: `${playerCombatant.name} Dodges and focuses on defense.` }));
  ...
  return <main className="app-shell">
    ...
    {pendingImport && reviewCharacter && <div className="import-review-backdrop">
      <form className="import-review" onSubmit={confirmReviewedImport} role="dialog" aria-modal="true" aria-labelledby="import-review-title">
        ...
        {(reviewCharacter.resources?.some((resource) => resource.kind === "spell-slot")) && <div className="import-attacks"><span>Imported spell pools</span><p>{reviewCharacter.resources.filter((resource) => resource.kind === "spell-slot").map((resource) => `Level ${resource.level} ${resource.current}/${resource.maximum}`).join(" · ")}</p></div>}
        {(reviewCharacter.resources?.some((resource) => resource.kind === "generic")) && <div className="import-attacks"><span>Imported feature resources</span><p>{reviewCharacter.resources.filter((resource) => resource.kind === "generic").map((resource) => `${resource.name} ${resource.current}/${resource.maximum} · ${resource.recovery === "short-rest" ? "Short or Long Rest" : resource.recovery === "long-rest" ? "Long Rest" : "special recovery"}${resource.sourceFeatureName ? ` · from ${resource.sourceFeatureName}` : ""}`).join(" · ")}</p><p>Only explicit counters with a matching verified feature and recovery rule become executable. Ambiguous counters remain descriptive.</p></div>}
        {(reviewCharacter.spells?.length ?? 0) > 0 && <div className="import-attacks"><span>Imported spells</span><p><b>Verified:</b> {reviewCharacter.spells?.filter((spell) => !spell.unsupportedReason).map((spell) => `${spell.name} (${spell.level === 0 ? "cantrip" : `level ${spell.level}`})`).join(" · ") || "None"}</p><p><b>Descriptive only:</b> {reviewCharacter.spells?.filter((spell) => spell.unsupportedReason).map((spell) => `${spell.name} (${spell.level === 0 ? "cantrip" : `level ${spell.level}`})`).join(" · ") || "None"}. Descriptive spells remain unavailable until linked to verified mechanics.</p></div>}
        {((reviewCharacter.equipmentRules?.length ?? 0) > 0 || (reviewCharacter.profile?.features?.some((feature) => feature.executableActionId || feature.executablePassiveId || feature.executableTriggerId))) && <div className="import-attacks"><span>Verified import links</span><p><b>Equipment:</b> {reviewCharacter.equipmentRules?.map((rule) => `${rule.name} (${rule.equipped ? "active" : "carried"})`).join(" · ") || "None"}</p><p><b>Character features:</b> {reviewCharacter.profile?.features?.filter((feature) => feature.executableActionId || feature.executablePassiveId || feature.executableTriggerId).map((feature) => feature.name).join(" · ") || "None"}. Unmatched records remain descriptive.</p></div>}
        {reviewCharacter.source.extractionAssessment && <section className="import-playability" aria-label="PDF extraction confidence">
          ...
          {(reviewCharacter.profile?.species || reviewCharacter.profile?.background || reviewCharacter.profile?.playerName) && <p><b>Identity:</b> {[reviewCharacter.profile.playerName ? `Player ${reviewCharacter.profile.playerName}` : "", reviewCharacter.profile.species, reviewCharacter.profile.background].filter(Boolean).join(" · ")}</p>}
          {reviewCharacter.profile?.senses && <p><b>Senses:</b> {Object.entries(reviewCharacter.profile.senses).map(([sense, value]) => `${sense.replace(/([A-Z])/g, " $1").toLowerCase()} ${value}`).join(" · ")}</p>}
          {reviewCharacter.profile?.skills && <p><b>Skills:</b> {Object.entries(reviewCharacter.profile.skills).map(([skill, modifier]) => `${skill} ${modifier >= 0 ? "+" : ""}${modifier}`).join(" · ")}</p>}
          {reviewCharacter.profile?.spellcasting && <p><b>Spellcasting:</b> {reviewCharacter.profile.spellcasting.ability} · save DC {reviewCharacter.profile.spellcasting.saveDc} · attack {reviewCharacter.profile.spellcasting.attackBonus >= 0 ? "+" : ""}{reviewCharacter.profile.spellcasting.attackBonus}</p>}
        </section>}
    ...
    <section className="workspace">
      ...
      <section className="combat-area">
        ...
        <section className="state-tray" aria-label="Character resources and temporary effects">
          <div className="resource-tracker"><div><span className="eyebrow">Combat resources</span><h3>Uses and carried equipment</h3></div><div className="resource-pills">{playerCombatant.resources.map((resource) => <div key={resource.id}><span>{resource.kind === "spell-slot" ? `Level ${resource.level} slots` : resource.name}</span><strong>{resource.current}/{resource.maximum}</strong></div>)}{playerCombatant.inventory.map((item) => <div key={`inventory-${item.id}`}><span>{item.name}</span><strong>{item.current}/{item.maximum}</strong>{item.spellcastingFocusFor && <small>{hasSpellcastingFocus(encounter, playerCombatant.id, item.spellcastingFocusFor) ? `${item.spellcastingFocusFor} focus available` : "Focus unavailable"}</small>}{item.tool && <small>{item.tool.proficient ? "Tool proficiency ready" : "Ability modifier only"}</small>}{item.lightSource && <div className="light-source-controls"><small>{item.lightSource.mode} · {item.lightSource.fuelMinutesRemaining} min fuel</small><div><button type="button" disabled={item.lightSource.mode === "bright"} onClick={() => changeLightSource(item.id, "bright")}>Open</button><button type="button" disabled={item.lightSource.mode === "hooded"} onClick={() => changeLightSource(item.id, "hooded")}>Hood</button><button type="button" disabled={item.lightSource.mode === "off"} onClick={() => changeLightSource(item.id, "off")}>Off</button></div></div>}</div>)}{playerCombatant.damageResistances.map((type) => <div key={`resistance-${type}`}><span>Damage resistance</span><strong>{type}</strong><small>Matching damage is halved, rounded down, and shown in the combat log.</small></div>)}{!playerCombatant.resources.length && !playerCombatant.inventory.length && !playerCombatant.damageResistances.length && <p>No tracked resources imported.</p>}</div>{outcome === "victory" && <div className="rest-recovery"><span>Post-encounter recovery</span><div><button type="button" onClick={() => recoverAfterRest("short-rest")}>Recover after Short Rest</button><button type="button" onClick={() => recoverAfterRest("long-rest")}>Recover after Long Rest</button></div>{character.id === "surina-daardendrian" ? <><button type="button" onClick={() => recoverAfterRest("short-rest", true)}>Short Rest · Roll 1d10 + CON for healing ({encounter.recoveryState?.hitDiceRemaining ?? character.recoveryState?.hitDiceRemaining ?? 1} Hit Die left)</button><small>Safe, uninterrupted downtime: Short Rest advances 1 hour; Long Rest includes 8 hours with sleep and any required 16-hour waiting period since your previous Long Rest. Long Rest restores HP and your Hit Die. Source resource recovery remains unchanged.</small></> : <small>Refreshes only resources whose registered rules recover on that rest.</small>}</div>}</div>
        ...
        <section id="action-console" className="action-console">
          ...
          {encounter.effects.some(e => e.hidden && e.targetCombatantId === playerCombatant.id) && <button type="button" onClick={stopHidingBySpeaking}>Speak loudly / end Hide</button>}
