...
async function importFlattenedPdf(file: File, bytes: ArrayBuffer): Promise<ImportResult> {
  try {
    ...
    if (parsed) {
      ...
      const genericResourceCount = character.resources.filter((resource) => resource.kind === "generic").length;
      return importResult(character, "flattened-pdf", [`Flattened D&D Beyond sheet detected across ${parsed.extractionAssessment.pageCount} page${parsed.extractionAssessment.pageCount === 1 ? "" : "s"}. ${parsed.attacks.length} weapon attacks, ${parsed.spells.length} spells, ${parsed.resources.length} spell-slot pools, ${genericResourceCount} verified feature pools, ${parsed.profile.equipment?.length ?? 0} equipment records, ${parsed.profile.features?.length ?? 0} descriptive features, ${profileRecordCount} profile groups, and saving throw modifiers were extracted; review them before combat. ${editionWarning}`]);
    }
  } catch {
