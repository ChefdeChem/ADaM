...
export const actionCatalog: CombatAction[] = [
  { id: "attack", name: "Attack", cost: "action", description: "Make a melee attack against the selected creature. Weapon-specific ranges will come from the imported sheet.", rulesets: both, requiresTarget: true, rangeFeet: 5, requiresLineOfSight: true },
  ...
  { id: "help", name: "Help", cost: "action", description: "Assist the selected creature with a task or attack while within 5 feet.", rulesets: both, requiresTarget: true, rangeFeet: 5, requiresLineOfSight: true },
  ...
  { id: "influence", name: "Influence", cost: "action", description: "Attempt to influence a selected creature you can communicate with within 30 feet.", rulesets: ["dnd-2024"], requiresTarget: true, rangeFeet: 30, requiresLineOfSight: true },
...
export function validateAction(action: CombatAction, encounter: EncounterState): ActionValidation {
  ...
  if (action.requiresTarget) {
    ...
    if (action.requiresLineOfSight && !target.lineOfSight) return { legal: false, reason: `${target.target.name} is outside your line of sight.` };
    if (action.rangeFeet !== undefined && target.distanceFeet > action.rangeFeet) return { legal: false, reason: `${target.target.name} is ${target.distanceFeet} feet away; ${action.name} currently reaches ${action.rangeFeet} feet.` };
  }
  return { legal: true };
}
